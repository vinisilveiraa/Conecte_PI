<?php
/**
 * PÁGINA: DASHBOARD CUIDADOR
 * Área logada do cuidador com perfil e especialidades
 */

$current_page = 'dashboard-cuidador';
$user_type = 'cuidador';
$user_name = 'Ana Paula Santos';
$user_email = 'ana@email.com';
$user_phone = '(14) 99999-9999';
$user_cpf = '123.456.789-00';
$user_city = 'Jaú';
$user_neighborhood = 'Centro';
$user_street = 'Rua das Flores, 123';

$specialties = ['Banho', 'Curativo'];
?>

<!DOCTYPE html>
<html lang="pt-BR">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Dashboard Cuidador - Conecte</title>
  <link rel="stylesheet" href="../css/global.css">
  <link rel="stylesheet" href="../css/componentes.css">
  <link rel="stylesheet" href="../css/dashboard.css">
</head>
<body>
  <!-- NAVBAR -->
  <?php include '../components/navbar.php'; ?>

  <div class="dashboard-wrapper">
    <!-- SIDEBAR -->

    <?php include '../components/dashboard-sidebar-cuidador.php'; ?>

    <!-- MAIN CONTENT -->
    <main class="dashboard-content">
      <div class="container">
        <h1>Bem vindo, <span class="text-primary"><?php echo $user_name; ?></span>!</h1>

        <div class="dashboard-grid">
          <!-- PERFIL CARD -->
          <div class="profile-card">
            <div class="profile-avatar">A</div>
            <h3><?php echo $user_name; ?></h3>
            <p class="profile-type"><?php echo ucfirst($user_type); ?></p>
          </div>

          <!-- INFORMAÇÕES CARD -->
          <div class="info-card">
            <h3>Informações do Usuário</h3>
            
            <div class="info-item">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                <path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"></path>
                <polyline points="22 6 12 13 2 6"></polyline>
              </svg>
              <div>
                <label>E-mail</label>
                <p><?php echo $user_email; ?></p>
              </div>
            </div>

            <div class="info-item">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                <path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z"></path>
              </svg>
              <div>
                <label>Telefone</label>
                <p><?php echo $user_phone; ?></p>
              </div>
            </div>

            <div class="info-item">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                <circle cx="12" cy="12" r="1"></circle>
                <path d="M12 1v6m0 6v6"></path>
                <path d="M4.22 4.22l4.24 4.24m2.12 2.12l4.24 4.24"></path>
                <path d="M1 12h6m6 0h6"></path>
                <path d="M4.22 19.78l4.24-4.24m2.12-2.12l4.24-4.24"></path>
                <path d="M12 19v6"></path>
                <path d="M19.78 19.78l-4.24-4.24m-2.12-2.12l-4.24-4.24"></path>
                <path d="M19 12h6"></path>
                <path d="M19.78 4.22l-4.24 4.24m-2.12 2.12l-4.24 4.24"></path>
              </svg>
              <div>
                <label>CPF</label>
                <p><?php echo $user_cpf; ?></p>
              </div>
            </div>

            <div class="info-item">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                <path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"></path>
                <circle cx="12" cy="10" r="3"></circle>
              </svg>
              <div>
                <label>Cidade</label>
                <p><?php echo $user_city; ?></p>
              </div>
            </div>

            <div class="info-item">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                <path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"></path>
                <polyline points="9 22 9 12 15 12 15 22"></polyline>
              </svg>
              <div>
                <label>Bairro</label>
                <p><?php echo $user_neighborhood; ?></p>
              </div>
            </div>

            <div class="info-item">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                <path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"></path>
                <polyline points="9 22 9 12 15 12 15 22"></polyline>
              </svg>
              <div>
                <label>Rua</label>
                <p><?php echo $user_street; ?></p>
              </div>
            </div>

            <button class="btn btn-primary btn-block">Atualizar Dados</button>
          </div>
        </div>
      </div>
    </main>
  </div>

  <script src="../js/main.js"></script>
</body>
</html>
